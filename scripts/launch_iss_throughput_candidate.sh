...
TABLES="${ISS_GATE_TABLES:-1}"
WORKERS="${ISS_GATE_INFERENCE_WORKERS:-$TABLES}"
CANARY_GAMES="${ISS_GATE_CANARY_GAMES:-20}"
RUNTIME="${ISS_THROUGHPUT_RUNTIME_ROOT:-/workspace/skatai-v2-runtime/iss/throughput-candidate-t${TABLES}-w${WORKERS}-v1}"
...
case "$TABLES" in
  1|2|4) ;;
  *) fail "tables-must-be-1-2-or-4:$TABLES" ;;
esac
case "$WORKERS" in
  1|2|3|4) ;;
...
case "$CANARY_GAMES" in
  ''|*[!0-9]*) fail "canary-games-not-integer:$CANARY_GAMES" ;;
esac
[ "$CANARY_GAMES" -le 10000 ] || fail "canary-games-out-of-range:$CANARY_GAMES"
...
export ISS_GATE_CANARY_GAMES="$CANARY_GAMES"
...
printf '%s ISS_THROUGHPUT_LAUNCH_OK head=%s pid=%s tables=%s workers=%s canary_games=%s\n' \
  "$(date -u +%FT%TZ)" "$head" "$$" "$TABLES" "$WORKERS" "$CANARY_GAMES" >>"$LOG"
