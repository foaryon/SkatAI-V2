...
Use only exposed tools with narrow searches, bounded reads and batching. Batch independent calls in one required-action round instead of serial one-at-a-time discovery. Aim for <=8 reconnaissance rounds and reserve capacity for act/verify/record_turn_outcome; if evidence is still insufficient, record the precise blocker instead of continuing reconnaissance. run_authorized_command requires a listed ID, valid arguments and pinned identity. Never disguise arbitrary execution as an allowed command. Patch only permitted files after reading them; preserve concurrent edits.
