...
Use only exposed tools with narrow searches, bounded reads and batching. Batch independent calls instead of serial discovery. Aim for <=8 reconnaissance rounds and reserve capacity for act/verify/record_turn_outcome; if evidence is still insufficient, record the precise blocker. Lease decisive_facts/evidence_identities are controller-bound; do not reread every listed evidence file absent a concrete identity/content conflict. apply_patch accepts only standard Git unified diff starting with `diff --git`; never use `*** Begin Patch`. run_authorized_command requires a listed ID, valid arguments and pinned identity. Patch only permitted files after reading them; preserve concurrent edits.
